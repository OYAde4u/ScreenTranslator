...
更新:2026-08-31(第 19 轮:修复框选画面放大)

## 第 19 轮改动(框选 DPI 修复)
1. **根因**:第 18 轮注释写错了——进程 system-DPI-aware 下 WPF 按 96DPI 布局,OS 会把整窗位图拉伸 scale 倍(125% → 1.25);框选窗口直接用物理 2560×1600 当 DIP 尺寸 → 实际渲染 3200×2000 被裁切,画面放大至右上、显示不全。覆盖层早有 ToDip 换算,框选窗口漏了;
2. **修复**:窗口 Left/Top/Width/Height、画布、提示条定位全部用 DIP(物理÷`DisplayLayout.SystemScale`);鼠标坐标(DIP)上报选区时乘回 scale → 物理像素;
3. **验证**:探针截屏确认冻结画面与桌面 1:1 对齐、无裁切无放大。

## 第 18 轮改动(框选黑屏修复)
1. **根因**:整屏 Opacity=0.25 窗口触发"全屏优化"独立翻页,绕过 DWM 合成 → 透明度丢失 → 整屏不透明黑;内缩 1px 无效(实测中心像素仍 (0,0,0))。本机规律总结:小窗 Opacity 正常,AllowsTransparency 不合成,大/全屏窗口一切透明机制都失效;
2. **方案**:改为截图静止画(截图工具同款)——进入框选时 `CaptureSnapshot` 截全屏,不透明全屏窗口显示冻结画面 + 元素级半透明高亮框(子元素 alpha 由 WPF 内部合成,不受窗口问题影响);屏幕视觉上完全不变,无闪烁无黑屏;
3. **附带简化**:选区直接上报物理像素虚拟屏坐标,MainWindow 只需减虚拟屏原点,删掉了 DIP/多屏 DPI 换算代码;
4. **验证**:探针截屏确认框选界面正确渲染(冻结桌面 + 提示条,无黑屏)。

## 第 17 轮改动(框选翻译)
1. **框选区域模式**:新增 `RegionSelectorWindow`(全屏 Opacity=0.25 暗化层 + 独立半透明高亮框窗口跟随拖动;Esc/右键取消;左键拖选 ≥20×12 DIP 生效)。本机不合成分层窗口,故全程走 Opacity 机制;
2. **管线分支**(`_region` 非 null):跳过脏区 diff,OCR 范围 = 框选矩形;翻译完成后**不画屏幕覆盖层**,译文 \n 连接后送入悬浮框;识别复用指纹泛化为任意矩形(`HashRegion(x,y,w,h)` 网格采样);
3. **悬浮框扩展**:宽度 400,`SizeToContent=Height`;新增第二行译文面板(深灰圆角 + 滚动 ≤280px,有译文自动展开);新增「选区/清选」钮(状态互转);主窗口新增「框选区域」按钮;选区完成后立即触发一次翻译;
4. 坐标换算:DIP 虚拟屏 → 物理,按选区中心所在显示器 Scale 换算(TODO: 多屏异 DPI 边缘近似);
5. 验证:build 0 错误;e2e/filter/ocr-hybrid 自检 exit=0;悬浮框上屏采样 mean=74.7/std=51(含按钮)正常。
