...
- 🎯 **原文位置覆盖**:OCR 精确定位文字坐标,译文取原文周围背景色原位替换,不遮画面其他内容;
...
- 🎛️ **悬浮状态框**:灰色半透明小窗置顶显示翻译进度与耗时,游戏时可直接立即翻译/开关自动触发/切换目标语言(可关);
- 🖱️ **框选区域模式(galgame 利器)**:拖选游戏对话框区域,只识别该区域,译文直接显示在悬浮框面板里,游戏画面零遮挡;
- 📚 **段落聚合翻译**:一句话被 OCR 拆成多行时先拼回整段再翻译,译文更通顺;多语言并行请求提速;
- 🖥️ **多显示器支持**:逐显示器截图与覆盖,任意 DPI 组合下坐标精确对齐;
- 🔤 **目标语言可选**:中文 / 英文 / 日文;自动过滤已为目标语言的内容与垃圾文本(网址、纯数字等);
- 🎨 **背景采样覆盖**:取文本外围一圈背景色平均值填充,纯色/半透明对话框背景上像原文被原位替换,观感最自然。
...
4. 通过下拉框切换 **目标语言**(中文/英文/日文);悬浮状态框上还有 ▶ 立即翻译 / 自动开关 / 语言循环 / × 隐藏;
